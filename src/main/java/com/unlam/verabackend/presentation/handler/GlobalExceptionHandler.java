...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> handleEntityNotFound(EntityNotFoundException ex) {
        log.error("Entity not found in database: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ex.getMessage());
    }
    ...
    public ResponseEntity<String> handleMessageDelivery(MessageDeliveryException ex) {
        log.error("Error crítico en la distribución del bus de mensajería asíncrono (STOMP/RTC): {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Fallo en la entrega del mensaje de tiempo real.");
    }
